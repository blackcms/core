...
namespace BlackCMS\JsValidation;

use BlackCMS\JsValidation\Javascript\JavascriptValidator;
use BlackCMS\JsValidation\Javascript\MessageParser;
use BlackCMS\JsValidation\Javascript\RuleParser;
use BlackCMS\JsValidation\Javascript\ValidatorHandler;
use BlackCMS\JsValidation\Support\DelegatedValidator;
use BlackCMS\JsValidation\Support\ValidationRuleParserProxy;
...
class JsValidatorFactory
{
    ...
    {
        $options["disable_remote_validation"] = empty(
            $options["disable_remote_validation"]
        )
            ? false
            : $options["disable_remote_validation"];
        $options["view"] = empty($options["view"])
            ? "core/js-validation:bootstrap"
            : $options["view"];
        $options["form_selector"] = empty($options["form_selector"])
            ? "form"
    ...
    protected function jsValidator(Validator $validator, $selector = null)
    {
        ...
        $selector = is_null($selector)
            ? $this->options["form_selector"]
            : $selector;

        $delegated = new DelegatedValidator(
            $validator,
            new ValidationRuleParserProxy($validator->getData())
        );
        $rules = new RuleParser($delegated, $this->getSessionToken());
        $messages = new MessageParser(
            $delegated,
            isset($this->options["escape"]) ? $this->options["escape"] : false
        );

        $jsValidator = new ValidatorHandler($rules, $messages);

        $manager = new JavascriptValidator(
            $jsValidator,
            compact("view", "selector", "remote")
        );
