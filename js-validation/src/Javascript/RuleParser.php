...
namespace BlackCMS\JsValidation\Javascript;

use BlackCMS\JsValidation\Support\DelegatedValidator;
use BlackCMS\JsValidation\Support\RuleListTrait;
use BlackCMS\JsValidation\Support\UseDelegatedValidatorTrait;
...
class RuleParser
{
    ...
    public function getRule($attribute, $rule, $parameters, $rawRule)
    {
        ...
        if ($isConditional || $isRemote) {
            [$attribute, $parameters] = $this->remoteRule(
                $attribute,
                $isConditional
            );
            $jsRule = self::REMOTE_RULE;
        } else {
            [$jsRule, $attribute, $parameters] = $this->clientRule(
                $attribute,
                $rule,
                $parameters
            );
    ...
    public function addConditionalRules($attribute, $rules = [])
    {
        foreach ((array) $attribute as $key) {
            $current = isset($this->conditional[$key])
    ...
    protected function remoteRule($attribute, $forceRemote)
    {
        ...
        $params = [$attrHtmlName, $this->remoteToken, $forceRemote];
    ...
    protected function getAttributeName($attribute)
    {
        ...
        if (count($attributeArray) > 1) {
            return $attributeArray[0] .
                "[" .
                implode("][", array_slice($attributeArray, 1)) .
                "]";
        }
