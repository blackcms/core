...
class ValidatorHandler
{
    ...
    protected function jsConvertRules($attribute, $rules, $includeRemote)
    {
        ...
        foreach ($rules as $rawRule) {
            ...
            [$jsAttribute, $jsRule, $jsParams] = $this->rules->getRule(
                $attribute,
                $rule,
                $parameters,
                $rawRule
    ...
    {
        return $jsRule &&
            ($includeRemote || $jsRule !== RuleParser::REMOTE_RULE);
    }
    ...
    {
        return !$this->validator->hasRule(
            $attribute,
            self::JS_VALIDATION_DISABLE
        );
    }
