...
use BlackCMS\JsValidation\Remote\Resolver;
use BlackCMS\JsValidation\Remote\Validator as RemoteValidator;
use Closure;
...
class RemoteValidationMiddleware
{
    ...
    public function __construct(ValidationFactory $validator, Config $config)
    {
        ...
        $this->field = $config->get(
            "core.js-validation.js-validation.remote_validation_field"
        );
        $this->escape = (bool) $config->get(
            "core.js-validation.js-validation.escape",
            false
        );
    }
    ...
    protected function wrapValidator()
    {
        ...
        $this->factory->extend(
            RemoteValidator::EXTENSION_NAME,
