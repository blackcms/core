...
class BDashboard {
    static loadWidget(el, url, data, callback) {
        ...
        const predefinedRange = widgetItem.find(
            "select[name=predefined_range]"
        );
            ...
            success: (res) => {
                ...
                if (!res.error) {
                    ...
                    if (el.find(".scroller").length !== 0) {
                        BlackCMS.callScroll(el.find(".scroller"));
                    }
                    ...
                    el.html(
                        '<div class="dashboard_widget_msg col-12"><p>' +
                            res.message +
                            "</p>"
                    );
                }
            ...
            Sortable.create(el, {
                ...
                onUpdate: () => {
                    ...
                    $.ajax({
                        ...
                        success: (res) => {
                            if (!res.error) {
                                BlackCMS.showSuccess(res.message);
                            } else {
    ...
    init() {
        ...
        $(document).on(
            ...
            (event) => {
                ...
                $("#hide-widget-confirm-bttn").data(
                    "id",
                    $(event.currentTarget).closest(".widget_item").prop("id")
                );
        ...
        list_widgets.on("click", ".page_next, .page_previous", (e) => {
            ...
            if (href) {
                BDashboard.loadWidget(
                    $this.closest(".portlet").find(".portlet-body"),
                    href
                );
            }
        });
        ...
        list_widgets.on("change", ".number_record .numb", (e) => {
            ...
            if (!isNaN(paginate) && paginate > 0) {
                BDashboard.loadWidget(
                    $this.closest(".portlet").find(".portlet-body"),
                    $this.closest(".widget_item").attr("data-url"),
                    { paginate: paginate }
                );
            } else {
                BlackCMS.showError("Please input a number!");
            }
        });
        ...
        $(document).on(
            ...
            (event) => {
                ...
                let icon = _self[0].querySelector("i");
                if (state === "expand") {
                    $portlet
                        .find(".portlet-body")
                    ...
                    BDashboard.loadWidget(
                        $portlet.find(".portlet-body"),
                        _self.closest(".widget_item").attr("data-url")
                    );
                } else {
                    $portlet
                        .find(".portlet-body")
        ...
        $(document).on(
            "change",
            ".portlet select[name=predefined_range]",
            (e) => {
                e.preventDefault();
                const $this = $(e.currentTarget);
                BDashboard.loadWidget(
