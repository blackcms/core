...
namespace BlackCMS\Dashboard\Supports;
...
class WidgetInstance
{
    ...
    public function init($widgets, $widgetSettings)
    {
        ...
        if (!$widget) {
            $widget = app(DashboardWidgetInterface::class)->firstOrCreate([
                "name" => $this->key,
            ]);
        }
        ...
        if ($this->type === "widget") {
            ...
            $settings = array_merge(
                $widgetSetting && $widgetSetting->settings
            ...
            if (empty($widgetSetting) ||
                array_key_exists($widgetSetting->order, $widgets)
            ) {
                $widgets[] = $data;
            } else {
        ...
        $widgets[$this->key] = [
            ...
            "type" => $this->type,
            "view" => view(
                "core/dashboard::widgets.stats",
                compact("widget", "widgetSetting")
            )->render(),
        ];
    ...
    public function getPredefinedRangesDefault(): array
    {
        ...
        return [
            [
                "key" => "today",
                "label" => trans(
                    "core/dashboard::dashboard.predefined_ranges.today"
                ),
            ...
            [
                "key" => "yesterday",
                "label" => trans(
                    "core/dashboard::dashboard.predefined_ranges.yesterday"
                ),
            ...
            [
                "key" => "this_week",
                "label" => trans(
                    "core/dashboard::dashboard.predefined_ranges.this_week"
                ),
            ...
            [
                "key" => "last_7_days",
                "label" => trans(
                    "core/dashboard::dashboard.predefined_ranges.last_7_days"
                ),
                "startDate" => now()
                    ->subDays(7)
                    ->startOfDay(),
            ...
            [
                "key" => "this_month",
                "label" => trans(
                    "core/dashboard::dashboard.predefined_ranges.this_month"
                ),
            ...
            [
                "key" => "last_30_days",
                "label" => trans(
                    "core/dashboard::dashboard.predefined_ranges.last_30_days"
                ),
                "startDate" => now()
                    ->subDays(29)
                    ->startOfDay(),
            ...
            [
                "key" => "this_year",
                "label" => trans(
                    "core/dashboard::dashboard.predefined_ranges.this_year"
                ),
    ...
    public function getFilterRange(?string $filterRangeInput)
    {
        ...
        $predefinedRangeFound = $predefinedRanges->firstWhere(
            "key",
            $filterRangeInput
        );
    ...
    {
        $widget = app(DashboardWidgetInterface::class)->getFirstBy([
            "name" => $widgetName,
        ]);
        ...
        $widgetSetting = app(
            DashboardWidgetSettingInterface::class
        )->firstOrCreate([
            "widget_id" => $widget->id,
            "user_id" => Auth::user()->getKey(),
        ]);

        $widgetSetting->settings = array_merge(
            (array) $widgetSetting->settings,
            $settings
        );
