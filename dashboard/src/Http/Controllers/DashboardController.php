...
namespace BlackCMS\Dashboard\Http\Controllers;
...
class DashboardController extends BaseController
{
    ...
    public function getDashboard(Request $request)
    {
        ...
        $availableWidgetIds = collect($widgetData)
            ->pluck("id")
            ->all();
        ...
        return view(
            "core/dashboard::list",
            compact("widgets", "userWidgets", "statWidgets")
        );
    }
     ...
     */
    public function postEditWidgetSettingItem(
        Request $request,
        ...
        try {
            ...
            if (!$widget) {
                return $response
                    ->setError()
                    ->setMessage(
                        trans("core/dashboard::dashboard.widget_not_exists")
                    );
            }
            ...
            $widgetSetting->settings = array_merge(
                ...
                [
                    $request->input("setting_name") => $request->input(
                        "setting_value"
                    ),
                ]
            );
     ...
     * @return BaseHttpResponse
     */
    public function postUpdateWidgetOrder(
        Request $request,
        BaseHttpResponse $response
    ) {
        ...
        return $response->setMessage(
            trans("core/dashboard::dashboard.update_position_success")
        );
    }
    ...
    public function getHideWidget(Request $request, BaseHttpResponse $response)
    {
        ...
        return $response->setMessage(
            trans("core/dashboard::dashboard.hide_success")
        );
    }
     ...
     * @return BaseHttpResponse
     */
    public function postHideWidgets(
        Request $request,
        BaseHttpResponse $response
    ) {
