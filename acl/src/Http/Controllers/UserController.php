...
namespace BlackCMS\ACL\Http\Controllers;

use Assets;
use BlackCMS\Base\Events\UpdatedContentEvent;
use BlackCMS\Media\Services\ThumbnailService;
use File;
use Illuminate\Contracts\View\Factory;
use Illuminate\Http\RedirectResponse;
use Illuminate\Support\Facades\Auth;
use BlackCMS\ACL\Forms\PasswordForm;
use BlackCMS\ACL\Forms\ProfileForm;
use BlackCMS\ACL\Forms\UserForm;
use BlackCMS\ACL\Tables\UserTable;
use BlackCMS\ACL\Http\Requests\CreateUserRequest;
use BlackCMS\ACL\Http\Requests\UpdatePasswordRequest;
use BlackCMS\ACL\Http\Requests\UpdateProfileRequest;
use BlackCMS\ACL\Models\UserMeta;
use BlackCMS\ACL\Repositories\Interfaces\RoleInterface;
use BlackCMS\ACL\Repositories\Interfaces\UserInterface;
use BlackCMS\ACL\Services\ChangePasswordService;
use BlackCMS\ACL\Services\CreateUserService;
...
use BlackCMS\ACL\Http\Requests\AvatarRequest;
use Exception;
...
class UserController extends BaseController
{
    ...
    public function index(UserTable $dataTable)
    {
        ...
        Assets::addScripts(["bootstrap-editable", "jquery-ui"])->addStyles([
            "bootstrap-editable",
        ]);
    ...
    public function store(
        ...
        event(new CreatedContentEvent(USER_MODULE_NAME, $request, $user));
        ...
        try {
            ...
            if (!$request->user()->isSuperUser() && $user->isSuperUser()) {
                return $response
                    ->setError()
                    ->setMessage(
                        trans("core/acl::users.cannot_delete_super_user")
                    );
            }

            $this->userRepository->delete($user);
            event(new DeletedContentEvent(USER_MODULE_NAME, $request, $user));
        ...
        foreach ($ids as $id) {
            if ($request->user()->getKey() == $id) {
                return $response
                    ->setError()
                    ->setMessage(
                        trans("core/acl::users.delete_user_logged_in")
                    );
            }
            try {
                $user = $this->userRepository->findOrFail($id);
                ...
                $this->userRepository->delete($user);
                event(
                    new DeletedContentEvent(USER_MODULE_NAME, $request, $user)
                );
            } catch (Exception $exception) {
                return $response
                    ->setError()
    ...
    public function getUserProfile(
        ...
        return view(
            ...
            compact("user", "form", "passwordForm", "canChangeProfile")
        );
    }
    ...
    public function postUpdateProfile(
        ...
        if (($currentUser->hasPermission("users.update-profile") &&
        ...
        event(new UpdatedContentEvent(USER_MODULE_NAME, $request, $user));

        return $response->setMessage(
            trans("core/acl::users.update_profile_success")
        );
    }
    ...
    public function postChangePassword(
        ...
        if ($result instanceof Exception) {
            return $response->setError()->setMessage($result->getMessage());
        }

        return $response->setMessage(
            trans("core/acl::users.password_update_success")
        );
    }
    ...
    public function postAvatar(
        ...
        try {
            ...
            $thumbnailService
                ->setImage(MediaManagement::getRealPath($file->url))
            ...
            return $response
                ...
                ->setData(["url" => MediaManagement::url($file->url)]);
        } catch (Exception $exception) {
     ...
     * @return RedirectResponse
     */
    public function toggleSidebarMenu(
        Request $request,
        BaseHttpResponse $response
    ) {
