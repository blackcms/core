...
namespace BlackCMS\ACL\Forms;

use Assets;
use BlackCMS\ACL\Http\Requests\RoleCreateRequest;
...
class RoleForm extends FormAbstract
{
    ...
    public function buildForm()
    {
        ...
        $this->setupModel(new Role())
            ...
            ->add("color", "text", [
                "label" => trans("core/base::forms.color"),
                "label_attr" => ["class" => "control-label"],
                "attr" => [
                    "placeholder" => trans(
                        "core/base::forms.color_placeholder"
                    ),
                    "data-counter" => 120,
                ],
            ])
            ...
            ->addMetaBoxes([
                "permissions" => [
                    ...
                    "content" => view(
                        "core/acl::roles.permissions",
                        compact("active", "flags", "children")
                    )->render(),
                ],
            ])
            ->setActionButtons(
                view("core/acl::roles.actions", [
                    "role" => $this->getModel(),
                ])->render()
            );
    }
    ...
    protected function getAvailablePermissions(): array
    {
        ...
        $types = ["core", "packages", "addons"];

        foreach ($types as $type) {
            $permissions = array_merge(
                $permissions,
                $this->getAvailablePermissionForEachType($type)
            );
        }
    ...
    protected function getAvailablePermissionForEachType($type)
    {
        ...
        foreach (scan_folder(platform_path($type)) as $module) {
            $configuration = config(
                strtolower($type . "." . $module . ".permissions")
            );
            if (!empty($configuration)) {
                foreach ($configuration as $config) {
                    $permissions[$config["flag"]] = $config;
