...
namespace BlackCMS\ACL\Commands;
...
class UserCreateCommand extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $user->first_name = $this->askWithValidate(
                "Enter first name",
                "required|min:2|max:60"
            );
            $user->last_name = $this->askWithValidate(
                "Enter last name",
                "required|min:2|max:60"
            );
            $user->email = $this->askWithValidate(
                "Enter email address",
                "required|email|unique:users,email"
            );
            $user->username = $this->askWithValidate(
                "Enter username",
                "required|min:4|max:60|unique:users,username"
            );
            $user->password = bcrypt(
                $this->askWithValidate(
                    "Enter password",
     ...
     * @return string
     */
    protected function askWithValidate(
        string $message,
        string $rules,
        $secret = false
    ): string {
