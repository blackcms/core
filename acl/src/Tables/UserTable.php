...
namespace BlackCMS\ACL\Tables;

use BaseHelper;
use Illuminate\Support\Facades\Auth;
use BlackCMS\ACL\Enums\UserStatusEnum;
use BlackCMS\ACL\Repositories\Interfaces\ActivationInterface;
use BlackCMS\ACL\Repositories\Interfaces\UserInterface;
use BlackCMS\ACL\Services\ActivateUserService;
...
class UserTable extends TableAbstract
{
    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn("username", function ($item) {
                ...
                return Html::link(
                    route("users.profile.view", $item->id),
                    $item->username
                );
            })
            ...
            ->editColumn("role_name", function ($item) {
                ...
                return view("core/acl::users.partials.role", [
                    "item" => $item,
                ])->render();
            })
            ->editColumn("super_user", function ($item) {
                return $item->super_user
            ...
            ->addColumn("operations", function ($item) {
                ...
                return apply_filters(
                    ...
                    $action .
                        view("core/acl::users.partials.actions", [
                            "item" => $item,
                        ])->render(),
                    $item
                );
    ...
    public function getBulkChanges(): array
    {
        return [
            ...
            "status" => [
                ...
                "validate" =>
                    "required|in:" . implode(",", UserStatusEnum::values()),
            ],
            "created_at" => [
                "title" => trans("core/base::tables.created_at"),
                "type" => "date",
    ...
    public function saveBulkChanges(
        ...
        if (app()->environment("demo")) {
            throw new Exception(
                trans("core/base::system.disabled_in_demo_mode")
            );
        }
        ...
        if ($inputKey === "status") {
            ...
            foreach ($ids as $id) {
                if ($inputValue == UserStatusEnum::DEACTIVATED &&
                    Auth::id() == $id
                ) {
                    $hasWarning = true;
                }
                ...
                event(
                    new UpdatedContentEvent(USER_MODULE_NAME, request(), $user)
                );
            }

            if ($hasWarning) {
                throw new Exception(
                    trans("core/acl::users.lock_user_logged_in")
                );
            }
