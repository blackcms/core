...
namespace BlackCMS\Table\Abstracts;
...
abstract class TableAbstract extends DataTable
{
    ...
    public function __construct(Datatables $table, UrlGenerator $urlGenerator)
    {
        ...
        if (!$this->getOption("id")) {
            $this->setOption(
                "id",
                strtolower(Str::slug(Str::snake(get_class($this))))
            );
        }
        ...
        if (!$this->getOption("class")) {
            $this->setOption(
                "class",
                "table table-striped table-hover vertical-middle"
            );
        }
        ...
        return $this->builder()
            ...
            ->parameters([
                ...
                "language" => [
                    ...
                    "lengthMenu" => Html::tag("span", "_MENU_", [
                        "class" => "dt-length-style",
                    ])->toHtml(),
                    "search" => "",
                    "searchPlaceholder" => trans("core/table::table.search"),
                    "zeroRecords" => trans("core/base::tables.no_record"),
                    "processing" => Html::image(
                        url(
                            "vendor/core/core/base/images/loading-spinner-blue.gif"
                        )
                    ),
                    "paginate" => [
                        "next" => trans("pagination.next"),
    ...
    public function getColumns(): array
    {
        ...
        foreach ($columns as $key => &$column) {
            $column["class"] =
                Arr::get($column, "class") . " column-key-" . $key;
        }
    ...
    protected function getOperations(
        ...
        return apply_filters(
            ...
            table_actions($edit, $delete, $item, $extra),
            $item,
            $edit,
            $delete,
            $extra
    ...
    public function applyScopes($query)
    {
        ...
        if ($request->has("filter_columns") &&
            $request->input("filter_table_id") == $this->getOption("id")
        ) {
            $requestFilters = [];
        ...
        foreach ($requestFilters as $requestFilter) {
            if (isset($requestFilter["column"]) &&
                !empty($requestFilter["column"])
            ) {
        ...
        return parent::applyScopes(
            apply_filters(BASE_FILTER_TABLE_QUERY, $query)
        );
    }
        ...
        switch ($type) {
            ...
            case "customSelect":
                ...
                $attributes["placeholder"] = trans(
                    "core/table::table.select_option"
                );
            ...
            case "select-search":
                $attributes["class"] =
                    $attributes["class"] . " select-search-full";
                $attributes["placeholder"] = trans(
                    "core/table::table.select_option"
                );
                $html = Form::customSelect(
            ...
            case "date":
                ...
                $attributes["data-date-format"] = config(
                    "core.base.general.date_format.js.date"
                );
                $content = Form::text(
                    $inputName,
                    $value,
                ...
                $html = view(
                    "core/table::partials.date-field",
                    compact("content")
                )->render();
                break;
    ...
    public function saveBulkChangeItem(
        ...
        $item->{$inputKey} = $this->prepareBulkChangeValue(
            $inputKey,
            $inputValue
        );
    ...
    public function renderFilter(): string
    {
        ...
        return view(
            $this->filterTemplate,
            compact("columns", "class", "tableId", "requestFilters")
        )->render();
    }
    ...
    public function toJson($data, $escapeColumn = [], $mDataSupport = true)
    {
        ...
        return $data->escapeColumns($escapeColumn)->make($mDataSupport);
    }
