...
class EditorManagement {
            ...
            .then((editor) => {
                editor.plugins.get("FileRepository").createUploadAdapter = (
                    ...
                    return new CKEditorUploadAdapter(
                        loader,
                        MEDIA_MANAGEMENT_URL.media_upload_from_editor,
                        editor.t
                    );
                };
    ...
    getShortcodesAvailable(editor) {
        const $dropdown = $(editor)
            .parents(".form-group")
            .find(".add_shortcode_btn_trigger")
            ?.next(".dropdown-menu");
        const lists = [];
    ...
    uploadImageFromEditor(blobInfo, callback) {
        ...
        $.ajax({
            type: "POST",
            data: formData,
            url: MEDIA_MANAGEMENT_URL.media_upload_from_editor,
            processData: false,
            contentType: false,
            cache: false,
            success(res) {
    ...
    initTinyMce(element) {
        tinymce.init({
            ...
            resize: "vertical",
            plugins:
                "code autolink advlist visualchars link image media table charmap hr pagebreak nonbreaking hanbiroclip anchor insertdatetime lists textcolor wordcount imagetools  contextmenu  visualblocks",
            ...
            images_upload_url: MEDIA_MANAGEMENT_URL.media_upload_from_editor,
    ...
    init() {
        ...
        $(document).on("click", ".show-hide-editor-btn", (event) => {
            ...
            if ($result.hasClass("editor-ckeditor")) {
                if (
                    this.CKEDITOR[editorInstance] &&
                    typeof this.CKEDITOR[editorInstance] !== "undefined"
                ) {
                    this.CKEDITOR[editorInstance].destroy();
                    this.CKEDITOR[editorInstance] = null;
                    $(".editor-action-item")
                        .not(".action-show-hide-editor")
                        .hide();
                } else {
    ...
    shortcodeCallback(params = {}) {
        ...
        $.ajax({
            ...
            success: (res) => {
                if (res.error) {
                    BlackCMS.showError(res.message);
                    return false;
                }
    ...
    manageShortCode() {
        ...
        $(".list-shortcode-items li a").on("click", function (event) {
            ...
            if ($(this).data("has-admin-config") == "1") {
                ...
                const editorInstance = $(".add_shortcode_btn_trigger").data(
                    "result"
                );
                ...
                if ($(".editor-ckeditor").length > 0) {
                    self.CKEDITOR[editorInstance].commands.execute(
                        "shortcode",
                        shortcode
                    );
                } else {
                    tinymce
                        .get(editorInstance)
                        .execCommand("mceInsertContent", false, shortcode);
                }
            }
        });
        ...
        $(".add_short_code_btn").on("click", function (event) {
            event.preventDefault();
            let formElement = $(".short_code_modal").find(
                ".short-code-data-form"
            );
            ...
            $.each(formData, function (name, value) {
                ...
                if (
                    (!shortcodeAttribute || shortcodeAttribute !== "content") &&
                    value
                ) {
                    name = name.replace("[]", "");
