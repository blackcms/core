...
class Layout {
        ...
        if (
            ...
            if (body.hasClass("page-sidebar-fixed")) {
                ...
                if (
                    ...
                    height =
                        App.getViewPort().height - headerHeight - footerHeight;
                } else {
    ...
    handleSidebarMenu() {
        ...
        $(".page-sidebar-mobile-offcanvas .responsive-toggler").on(
            "click",
            (e) => {
        ...
        if (this.$body.hasClass("page-sidebar-mobile-offcanvas")) {
            $(document).on("click", (e) => {
                if (
                    current.$body.hasClass("page-sidebar-mobile-offcanvas-open")
                ) {
                    if (
                        $(e.target).closest(
                            ".page-sidebar-mobile-offcanvas .responsive-toggler"
                        ).length === 0 &&
                        $(e.target).closest(".page-sidebar-wrapper").length ===
                            0
                    ) {
                        current.$body.removeClass(
        ...
        $(".page-sidebar-menu").on(
            ...
            (e) => {
                ...
                if (
                    App.getViewPort().width >= current.resBreakpointMd &&
                    !menu.attr("data-initialized") &&
                    current.$body.hasClass("page-sidebar-closed") &&
                    that.parent("li").parent(".page-sidebar-menu").length === 1
                ) {
                    return;
                }
                ...
                if (
                    App.getViewPort().width >= current.resBreakpointMd &&
                    that.parents(".page-sidebar-menu-hover-submenu").length ===
                        1
                ) {
    ...
    handleFixedSidebar() {
        ...
        if (
            ...
            menu.attr(
                "data-height",
                Layout._calculateFixedSidebarViewportHeight()
            );
            Layout.handleSidebarAndContentHeight();
        }
    }
        ...
        $(".full-height-content").each((index, el) => {
            ...
            if (target.hasClass("portlet")) {
                ...
                height =
                    ...
                    parseInt(
                        target.find(".portlet-body").css("padding-bottom")
                    ) -
                    5;
