...
$(() => {
    ...
    const $treeLoading = $(".tree-locations-container").find(".tree-loading");
    ...
    function loadTree(activeId) {
        ...
        if (activeId) {
            $treeWrapper
                .find('li[data-id="' + activeId + '"] .location-name:first')
                .addClass("active");
        }
    }
    ...
    $(document).on(
        "click",
        ".tree-locations-container .toggle-tree",
        function (e) {
            const $this = $(e.currentTarget);
            if ($this.hasClass("open-tree")) {
                $this.text($this.data("collapse"));
                $(".tree-locations-container")
    ...
    $(document).on("click", ".tree-locations-create", (event) => {
        event.preventDefault();
        const $this = $(event.currentTarget);
            ...
            success: (data) => {
                if (data.error) {
                    ...
                    reloadTree(activeId, function () {
                        ...
                        } else if ($(".tree-locations-create").length) {
                            $(".tree-locations-create").trigger("click");
                        } else {
        ...
        $.ajax({
            ...
            success: (data) => {
                if (data.error) {
                    ...
                    BlackCMS.showSuccess(data.message);
                    reloadTree();
                    let $createButton = $(".tree-locations-create");
