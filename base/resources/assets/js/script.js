class BlackCMS {
    constructor() {
        ...
        BlackCMS.initResources();
        BlackCMS.handleCounterUp();
        BlackCMS.initMediaIntegrate();
        if (BlackCMSVariables && BlackCMSVariables.authorized === "0") {
    ...
    static blockUI(options) {
        ...
        if (options.animate) {
            ...
            html =
                '<div class="loading-message ' +
                (options.boxed ? "loading-message-boxed" : "") +
                '"><img src="/vendor/core/core/base/images/loading-spinner-blue.gif" alt="loading"></div>';
            ...
            html =
                '<div class="loading-message ' +
                (options.boxed ? "loading-message-boxed" : "") +
                '"><img src="/vendor/core/core/base/images/loading-spinner-blue.gif" alt="loading"><span>&nbsp;&nbsp;' +
                (options.message ? options.message : "LOADING...") +
                "</span></div>";
        }
    ...
    static showNotice(messageType, message, messageHeader = "") {
        ...
        if (!messageHeader) {
            switch (messageType) {
                case "error":
                    messageHeader =
                        BlackCMSVariables.languages.notices_msg.error;
                    break;
                case "success":
                    messageHeader =
                        BlackCMSVariables.languages.notices_msg.success;
                    break;
            }
        }
    ...
    static handleError(data) {
        if (typeof data.errors !== "undefined" && !_.isArray(data.errors)) {
            BlackCMS.handleValidationError(data.errors);
        } else {
            if (typeof data.responseJSON !== "undefined") {
                if (typeof data.responseJSON.errors !== "undefined") {
                    if (data.status === 422) {
                        BlackCMS.handleValidationError(
                            data.responseJSON.errors
                        );
                    }
                } else if (typeof data.responseJSON.message !== "undefined") {
                    BlackCMS.showError(data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            BlackCMS.showError(item);
                        });
                    });
                }
            } else {
                BlackCMS.showError(data.statusText);
            }
        }
    }
    ...
    countCharacter() {
            ...
            settings = $.extend(
                {
                    ...
                    format:
                        "(%1 " +
                        BlackCMSVariables.languages.system.character_remain +
                        ")",
                    pulse: true,
                    delay: 0,
                },
        ...
        $(document).on(
            ...
            (event) => {
                $(event.currentTarget).charCounter(
                    $(event.currentTarget).data("counter"),
                    {
    ...
    static initResources() {
        if (jQuery().select2) {
            ...
            $.each(
                $(document).find(".select-search-full"),
                function (index, element) {
                    let options = {
                        width: "100%",
                    };
            ...
            $("select[multiple].select-sorting").on(
                "select2:select",
                function (evt) {
            ...
            $.each(
                ...
                function (index, element) {
                    if ($(element).data("url")) {
                        let options = {
                            placeholder:
                                $(element).data("placeholder") || "--Select--",
                            minimumInputLength:
                                $(element).data("minimum-input") || 1,
                            width: "100%",
                            delay: 250,
                            ajax: {
            ...
            $.each(
                ...
                function (index, element) {
                    $(element).inputmask({
                        ...
                        groupSeparator:
                            $(element).data("thousands-separator") ?? ",",
        ...
        BlackCMS.initDatePicker(".datepicker");
        if (jQuery().mCustomScrollbar) {
            BlackCMS.callScroll($(".list-item-checkbox"));
        }
    ...
    static initMediaIntegrate() {
        if (jQuery().mediaManagement) {
            $(
                '[data-type="media-management-standard-alone-button"]'
            ).mediaManagement({
                multiple: false,
            ...
            $.each($(document).find(".btn_gallery"), function (index, item) {
                $(item).mediaManagement({
                    multiple: false,
                    filter:
                        $(item).data("action") === "select-image"
                            ? "image"
                            : "everything",
                    view_in: "all_media",
                    onSelectFiles: (files, $el) => {
            ...
            new MediaManagementStandAlone(".js-btn-trigger-add-image", {
                filter: "image",
                view_in: "all_media",
                onSelectFiles: (files, $el) => {
                    ...
                    _.forEach(files, (file) => {
                        let template = $(document)
                            .find("#gallery_select_image_template")
                            .html();

                        let imageBox = template.replace(
                            /__name__/gi,
                            $el.attr("data-name")
                        );
            ...
            new MediaManagementStandAlone(
                ".images-wrapper .btn-trigger-edit-gallery-image",
                {
                    filter: "image",
                    view_in: "all_media",
                    onSelectFiles: (files, $el) => {
                        ...
                        let $currentBox = $el
                            .closest(".gallery-image-item-handler")
                            .find(".image-box");
                        let $currentBoxList = $el.closest(
                        ...
                        _.forEach(files, (file, index) => {
                            ...
                            let imageBox = template.replace(
                                /__name__/gi,
                                $currentBox.find(".image-data").attr("name")
                            );
            ...
            $(document).on(
                "click",
                ".btn-trigger-remove-gallery-image",
                (event) => {
                    event.preventDefault();
                    $(event.currentTarget)
                        .closest(".gallery-image-item-handler")
                    ...
                    if (
                        ...
                        $(".default-placeholder-gallery-image").removeClass(
                            "hidden"
                        );
                    }
                }
        ...
        $("body").on(
            ...
            (event) => {
                ...
                if (portlet.hasClass("portlet-fullscreen")) {
                    ...
                    let height =
                        BlackCMS.getViewPort().height -
                        portlet.children(".portlet-title").outerHeight() -
                        parseInt(
                            portlet.children(".portlet-body").css("padding-top")
                        ) -
                        parseInt(
                            portlet
    ...
    static initCodeEditor(id, type = "css") {
        ...
        $("#wrapper_" + id).append(
            '<div class="handle-tool-drag" id="tool-drag_' + id + '"></div>'
        );
        ...
        $(".handle-tool-drag").mousedown((event) => {
            ...
            _self
                .attr(
                    "data-start_h",
                    _self.parent().find(".CodeMirror").height()
                )
                .attr("data-start_y", event.pageY);
            $("body")
                .attr("data-dragtool", _self.attr("id"))
                .on("mousemove", BlackCMS.onDragTool);
            $(window).on("mouseup", BlackCMS.onReleaseTool);
        });
    }
    ...
    static onDragTool(e) {
        ...
        $(ele)
            ...
            .css(
                "height",
                Math.max(200, start_h + e.pageY - $(ele).attr("data-start_y"))
            );
    }

    static onReleaseTool() {
        $("body").off("mousemove", BlackCMS.onDragTool);
        $(window).off("mouseup", BlackCMS.onReleaseTool);
    }
