...
namespace BlackCMS\Base\Traits;
...
trait LoadAndPublishDataTrait
{
    ...
    protected function getConfigFilePath($file): string
    {
        ...
        if (!file_exists($file) && Str::contains($file, addon_path())) {
            $this->throwInvalidAddonError();
        }
        ...
        $this->publishes(
            [
                $this->getTranslationsPath() => resource_path(
                    "lang/vendor/" . $this->getDashedNamespace()
                ),
            ],
            "cms-lang"
        );
     ...
     * @throws Exception
     */
    protected function throwInvalidAddonError()
    {
        ...
        if ($from != $to) {
            throw new Exception(
                sprintf(
                    "Addon folder is invalid. Need to rename folder %s to %s",
                    $from,
                    $to
                )
            );
    ...
    {
        Helper::autoload(
            base_path() . "/" . $this->getDashedNamespace() . "/helpers"
        );
