...
namespace BlackCMS\Base\Http\Controllers;
...
use BlackCMS\Base\Supports\MembershipAuthorization;
use BlackCMS\Base\Supports\SystemManagement;
use BlackCMS\Base\Tables\InfoTable;
use BlackCMS\Table\TableBuilder;
use Exception;
...
class SystemController extends Controller
{
    ...
    public function getInfo(Request $request, TableBuilder $tableBuilder)
    {
        ...
        $matchPHPRequirement =
            version_compare(phpversion(), $requiredPhpVersion) > 0;
    ...
    {
        if ($lang != false &&
            array_key_exists($lang, Language::getAvailableLocales())
        ) {
            if (Auth::check()) {
                cache()->forget(
                    md5("cache-dashboard-menu-" . $request->user()->getKey())
                );
            }
