...
namespace BlackCMS\Base\Exceptions;
...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $exception)
    {
            ...
            return MediaManagement::responseError(
                trans("core/media::media.upload_failed", [
                    "size" => human_file_size(
                        MediaManagement::getServerConfigMaxUploadFileSize()
                    ),
                ])
            );
        }
                ...
                switch ($code) {
                    case 401:
                        return $response
                            ...
                            ->setMessage(
                                trans(
                                    "core/acl::permissions.access_denied_message"
                                )
                            )
                            ->setCode($code)
                            ->toResponse($request);
                    ...
                    case 403:
                        return $response
                            ...
                            ->setMessage(
                                trans(
                                    "core/acl::permissions.action_unauthorized"
                                )
                            )
                            ->setCode($code)
                            ->toResponse($request);
        ...
        if ($exception instanceof NotFoundHttpException &&
            setting("redirect_404_to_homepage", 0) == 1
        ) {
    ...
    public function report(Throwable $exception)
    {
        if ($this->shouldReport($exception) && !$this->isExceptionFromBot()) {
            if (!app()->isLocal() && !app()->runningInConsole()) {
                ...
                if (config(
                    "core.base.general.error_reporting.via_slack",
                    false
                ) &&
    ...
    {
        $ignoredBots = config(
            "core.base.general.error_reporting.ignored_bots",
            []
        );
    ...
    protected function getHttpExceptionView(HttpExceptionInterface $exception)
    {
        ...
        if (request()->is(BaseHelper::getAdminPrefix() . "/*") ||
