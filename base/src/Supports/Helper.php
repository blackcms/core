...
class Helper
{
     ...
     * @return boolean
     */
    public static function removeModuleFiles(
        string $module,
        $type = "packages"
    ): bool {
        $folders = [
    ...
    public static function executeCommand(
        ...
        if (!function_exists("proc_open")) {
            if (config("app.debug") &&
                config("core.base.general.can_execute_command")
            ) {
                throw new Exception(
                    trans("core/base::base.proc_close_disabled_error")
                );
            }
            return false;
        }
    ...
    public static function getCountryCodeByName(?string $countryName): ?string
    {
        ...
        $found = array_filter(self::countries(), function ($item) use (
            $countryName
        ) {
