...
class SystemManagement
{
     ...
     * @return array
     */
    public static function getPackagesAndDependencies(
        array $packagesArray
    ): array {
        $packages = [];
        foreach ($packagesArray as $key => $value) {
            ...
            if ($key !== "php" && File::exists($packageFile)) {
                ...
                $dependencies = array_key_exists("require", $dependenciesArray)
                    ? $dependenciesArray["require"]
                    : "No dependencies";
                $devDependencies = array_key_exists(
                    "require-dev",
                    $dependenciesArray
                )
                    ? $dependenciesArray["require-dev"]
                    : "No dependencies";
    ...
    public static function getSystemEnv(): array
    {
        return [
            ...
            "cache_dir_writable" => File::isReadable(
                base_path("bootstrap/cache")
            ),
            "lang_dir_writable" => File::isWritable(
    ...
    {
        return !empty(Request::server("HTTPS")) &&
