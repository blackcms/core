...
class PclZip
{
        ...
        while ($index < $p_size) {
            ...
            switch ($p_options_list[$index]) {
                    ...
                    if ($index + 1 >= $p_size) {
                        ...
                        PclZip::privErrorLog(
                            ...
                            'Missing parameter value for option \'' .
                                PclZipUtilOptionText($p_options_list[$index]) .
                                '"'
                        );
                ...
                case PCLZIP_OPT_TEMP_FILE_THRESHOLD:
                    ...
                    $v_result_list[$p_options_list[$index]] =
                        $v_value * 1048576;
                    $index++;
                    break;
                ...
                case PCLZIP_OPT_BY_NAME:
                    ...
                    // ----- Get the value
                    if (is_string($p_options_list[$index + 1])) {
                        $v_result_list[$p_options_list[$index]][0] =
                            $p_options_list[$index + 1];
                ...
                case PCLZIP_OPT_BY_INDEX:
                    ...
                    for ($jndex = 0; $jndex < count($v_work_list); $jndex++) {
                        ...
                        if ($v_size_item_list == 1) {
                            ...
                            $v_result_list[$p_options_list[$index]][$jndex][
                                "start"
                            ] = $v_item_list[0];
                            $v_result_list[$p_options_list[$index]][$jndex][
                                "end"
                        ...
                        // ----- Look for list sort
                        if ($v_result_list[$p_options_list[$index]][$jndex][
                                "start"
                            ] < $v_sort_value
                        ) {
                            $v_sort_flag = true;
        ...
        for ($index = 0; $index < count($p_filedescr_list); $index++) {
            ...
            if ($v_descr["type"] == "folder") {
                ...
                if ($v_folder_handler = @opendir($v_descr["filename"])) {
                    while (($v_item_handler = @readdir($v_folder_handler)) !==
                        false
                    ) {
                        // ----- Skip '.' and '..'
    ...
    public function privAddList($p_filedescr_list, &$p_result_list, &$p_options)
    {
        ...
        // ----- Add the files
        $v_header_list = [];
        if (($v_result = $this->privAddFileList(
            $p_filedescr_list,
            $v_header_list,
            $p_options
        )) != 1
        ...
        for ($index = 0, $v_count = 0; $index < count($v_header_list); $index++
            ...
            if ($v_header_list[$index]["status"] == "ok") {
                if (($v_result = $this->privWriteCentralFileHeader(
                    $v_header_list[$index]
                )) != 1
                ) {
                    // ----- Return
    ...
    public function privAddFileList(
        $p_filedescr_list,
        &$p_result_list,
        &$p_options
    ) {
        ...
        for ($jndex = 0; $jndex < count($p_filedescr_list) && $v_result == 1; $jndex++
            ...
            // ----- Check the filename
            if ($p_filedescr_list[$jndex]["type"] != "virtual_file" &&
                !file_exists($p_filedescr_list[$jndex]["filename"])
            ) {
                PclZip::privErrorLog(
                    PCLZIP_ERR_MISSING_FILE,
                    'File \'' .
            ...
            if ($p_filedescr_list[$jndex]["type"] == "file" ||
                $p_filedescr_list[$jndex]["type"] == "virtual_file" ||
                ($p_filedescr_list[$jndex]["type"] == "folder" &&
                    (!isset($p_options[PCLZIP_OPT_REMOVE_ALL_PATH]) ||
                        !$p_options[PCLZIP_OPT_REMOVE_ALL_PATH]))
            ) {
                // ----- Add the file
                $v_result = $this->privAddFile(
                    $p_filedescr_list[$jndex],
                    $v_header,
                    $p_options
                );
                if ($v_result != 1) {
    ...
    public function privWriteCentralHeader(
        $p_nb_entries,
        $p_size,
        $p_offset,
        $p_comment
    ...
    public function privExtractByRule(
        &$p_file_list,
        $p_path,
        $p_remove_path,
        $p_remove_all_path,
        ...
        for ($index = 0, $v_nb_extracted = 0; $index < $v_central_dir["entries"]; $index++
            ...
            if (isset($p_options[PCLZIP_OPT_BY_NAME]) &&
                ...
                for ($jndex = 0; $jndex < count($p_options[PCLZIP_OPT_BY_NAME]) &&
                    !$v_extract; $jndex++
                ) {
                    // ----- Look for a directory
                ...
                */
                ...
                for ($jndex = $jndex_start; $jndex < count($p_options[PCLZIP_OPT_BY_INDEX]) &&
                    !$v_extract; $jndex++
                ) {
                    if ($index >=
                            $p_options[PCLZIP_OPT_BY_INDEX][$jndex]["start"] &&
                        $index <= $p_options[PCLZIP_OPT_BY_INDEX][$jndex]["end"]
                    ) {
                        $v_extract = true;
                    }
                    ...
                    if ($p_options[PCLZIP_OPT_BY_INDEX][$jndex]["start"] >
                        $index
                    ) {
                        break;
                    }
            ...
            if ($v_extract && ($v_header["flag"] & 1) == 1) {
                ...
                if (isset($p_options[PCLZIP_OPT_STOP_ON_ERROR]) &&
                    ...
                    PclZip::privErrorLog(
                        ...
                        'Unsupported encryption for filename \'' .
                            $v_header["stored_filename"] .
                            '"'
                    );
    ...
    public function privExtractFile(
        &$p_entry,
        $p_path,
        $p_remove_path,
        $p_remove_all_path,
        ...
        if ($p_remove_all_path == true) {
            ...
            if (PclZipUtilPathInclusion($p_remove_path, $p_entry["filename"]) ==
                2
            ) {
                // ----- Change the file status
                $p_entry["status"] = "filtered";
    ...
    public function privDeleteByRule(&$p_result_list, &$p_options)
    {
        ...
        for ($index = 0, $v_nb_extracted = 0; $index < $v_central_dir["entries"]; $index++
            ...
            if (($v_result = $this->privReadCentralFileHeader(
                $v_header_list[$v_nb_extracted]
            )) != 1
            ) {
                // ----- Close the zip file
            ...
            if (isset($p_options[PCLZIP_OPT_BY_NAME]) &&
                ...
                for ($jndex = 0; $jndex < count($p_options[PCLZIP_OPT_BY_NAME]) && !$v_found; $jndex++
                    ...
                    if (substr($p_options[PCLZIP_OPT_BY_NAME][$jndex], -1) ==
                        ...
                        if (strlen(
                            ...
                            $v_found = true;
                        } elseif (($v_header_list[$v_nb_extracted]["external"] &
                                0x00000010) ==
                ...
                */
                ...
                // ----- Look if the index is in the list
                for ($jndexndex = $jndex_start; $jndexndex < count($p_options[PCLZIP_OPT_BY_INDEX]) &&
                    !$v_found; $jndexndex++
                ) {
                    if ($index >=
                            $p_options[PCLZIP_OPT_BY_INDEX][$jndexndex][
                                "start"
                            ] &&
                        $index <=
                            $p_options[PCLZIP_OPT_BY_INDEX][$jndexndex]["end"]
                    ) {
                        $v_found = true;
                    }

                    if ($p_options[PCLZIP_OPT_BY_INDEX][$jndexndex]["start"] >
                        $index
                    ) {
                        break;
                    }
        ...
        if ($v_nb_extracted > 0) {
            ...
            for ($index = 0; $index < count($v_header_list); $index++) {
                ...
                // ----- Read the file header
                $v_local_header = [];
                if (($v_result = $this->privReadFileHeader($v_local_header)) !=
                    1
                ) {
                    // ----- Close the zip file
                    $this->privCloseFd();
                ...
                if (($v_result = $v_temp_zip->privWriteFileHeader(
                    $v_header_list[$index]
                )) != 1
                ) {
                    // ----- Close the zip file
            ...
            for ($index = 0; $index < count($v_header_list); $index++) {
                // ----- Create the file header
                if (($v_result = $v_temp_zip->privWriteCentralFileHeader(
                    $v_header_list[$index]
                )) != 1
                ) {
                    $v_temp_zip->privCloseFd();
    ...
    public function privMerge(&$p_archive_to_add)
    {
        ...
        if (($v_result = $p_archive_to_add->privReadEndCentralDir(
            $v_central_dir_to_add
        )) != 1
        ) {
            $this->privCloseFd();
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
    ...
    $index = 0;
    $jndexndex = 0;
    while ($index < $v_list_dir_size &&
        $jndexndex < $v_list_path_size &&
        $v_result
    ) {
        ...
        // ----- Compare the items
        if ($v_list_dir[$index] != $v_list_path[$jndexndex] &&
            $v_list_dir[$index] != "" &&
            $v_list_path[$jndexndex] != ""
        ) {
            $v_result = 0;
        }
    ...
    if ($v_result) {
        ...
        while ($jndexndex < $v_list_path_size &&
            $v_list_path[$jndexndex] == ""
        ) {
            $jndexndex++;
        }
