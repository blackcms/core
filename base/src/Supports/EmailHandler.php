...
use BlackCMS\Base\Jobs\SendMailJob;
...
class EmailHandler
{
    ...
    public function initVariable(): self
    {
        $this->variables["core"] = [
            ...
            "site_admin_email" => trans(
                "core/base::base.email_template.site_admin_email"
            ),
        ];
     ...
     * @return $this
     */
    public function addTemplateSettings(
        string $module,
        array $data,
        string $type = "addons"
    ): self {
        ...
        add_filter(
            ...
            function ($html) use ($module, $data, $type) {
                return $html .
                    view(
                        "core/setting::template-line",
                        compact("module", "data", "type")
                    )->render();
            },
            99
        );
     ...
     */
    public function templateEnabled(string $template, string $type = "addons")
    {
    ...
    public function send(
        ...
        try {
            ...
            if (setting(
                "using_queue_to_send_mail",
                config("core.base.general.send_mail_using_job_queue")
            )
            ) {
    ...
    public function prepareData(string $content, array $variables = []): string
    {
        ...
        if (!empty($content)) {
            $content = $this->replaceVariableValue(
                array_keys($this->variables["core"]),
                "core",
                $content
            );
    ...
    public function initVariableValues()
    {
        $this->variableValues["core"] = [
            ...
            "site_url" => url(""),
            "site_logo" => setting("admin_logo")
                ? MediaManagement::getImageUrl(setting("admin_logo"))
     ...
     * @return string
     */
    protected function replaceVariableValue(
        array $variables,
        string $module,
        string $content
    ): string {
        ...
        foreach ($variables as $variable) {
            ...
            foreach ($keys as $key) {
                $content = str_replace(
                    $key,
                    $this->getVariableValue($variable, $module),
                    $content
                );
            }
        }
    ...
    public function sendErrorException(Exception $exception)
    {
        try {
            ...
            $this->send(
                view(
                    "core/base::emails.error-reporting",
                    compact("url", "ex", "error")
                )->render(),
                $exception->getFile(),
    ...
    public function getTemplateContent(
        ...
        return get_setting_email_template_content(
            $type,
            $this->module,
            $template
        );
