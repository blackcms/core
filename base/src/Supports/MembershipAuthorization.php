...
class MembershipAuthorization
{
    ...
    public function authorize(): bool
    {
        try {
            ...
            $authorizeDate = $this->settingStore->get(
                "membership_authorization_at"
            );
            if (!$authorizeDate) {
                return $this->processAuthorize();
            }

            $authorizeDate = Carbon::createFromFormat(
                "Y-m-d H:i:s",
                $authorizeDate
            );
    ...
    {
        $this->client->post("http://pyor.org/membership/authorize", [
            "form_params" => [
                "website" => $this->url,
            ],
        ]);
