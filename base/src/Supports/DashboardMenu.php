...
class DashboardMenu
{
    ...
    public function registerItem(array $options): self
    {
        ...
        if (!$id && !app()->runningInConsole() && app()->isLocal()) {
            ...
            throw new RuntimeException(
                "Menu id not specified: " . $calledClass
            );
        }
        ...
        if (isset($this->links[$id]) &&
            ...
            $calledClass = isset(debug_backtrace()[1])
                ...
                : null;
            throw new RuntimeException(
                "Menu id already exists: " . $id . " on class " . $calledClass
            );
        }

        if (isset($this->links[$id])) {
            $options["children"] = array_merge(
                $options["children"],
                $this->links[$id]["children"]
            );
            $options["permissions"] = array_merge(
                $options["permissions"],
                $this->links[$id]["permissions"]
            );
        ...
        if ($options["parent_id"]) {
            if (!isset($this->links[$options["parent_id"]])) {
                $this->links[$options["parent_id"]] =
                    ["id" => $options["parent_id"]] + $defaultOptions;
            }

            $this->links[$options["parent_id"]]["children"][] = $options;

            $permissions = array_merge(
                $this->links[$options["parent_id"]]["permissions"],
                $options["permissions"]
            );
    ...
    public function getAll(): Collection
    {
        ...
        foreach ($links as $key => &$link) {
            if ($link["permissions"] &&
                !Auth::user()->hasAnyPermission($link["permissions"])
            ) {
            ...
            $link["active"] =
                ...
                (Str::contains($link["url"], $routePrefix) &&
                    !in_array($routePrefix, [
                        "//",
                        "/" . BaseHelper::getAdminPrefix(),
                    ]) &&
            ...
            foreach ($link["children"] as $subKey => $subMenu) {
                if ($subMenu["permissions"] &&
                    !Auth::user()->hasAnyPermission($subMenu["permissions"])
                ) {
                ...
                if ($currentUrl == $subMenu["url"] ||
                    Str::contains($currentUrl, $subMenu["url"])
                ) {
