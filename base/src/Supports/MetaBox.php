    ...
    public function addMetaBox(
        ...
        foreach (array_keys($this->metaBoxes[$reference]) as $a_context) {
            foreach (["high", "core", "default", "low"] as $a_priority) {
                if (!isset(
                    $this->metaBoxes[$reference][$a_context][$a_priority][
                            $id
                        ]
                )
                ) {
                ...
                // If a core box was previously added or removed by a addon, don't add.
                if ("core" == $priority) {
                    ...
                    if ("default" == $a_priority) {
                        $this->metaBoxes[$reference][$a_context]["core"][$id] =
                            $this->metaBoxes[$reference][$a_context]["default"][
                                $id
                            ];
                        unset(
                            $this->metaBoxes[$reference][$a_context]["default"][
            ...
            foreach (["high", "sorted", "core", "default", "low"] as $priority) {
                ...
                foreach ((array) $this->metaBoxes[$reference][$context][$priority] as $box) {
                    ...
                    $data .= view("core/base::elements.meta-box-wrap", [
                        "box" => $box,
                        "callback" => call_user_func_array($box["callback"], [
                            $object,
                            $reference,
                            $box,
                        ]),
