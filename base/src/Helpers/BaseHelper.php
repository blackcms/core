...
namespace BlackCMS\Base\Helpers;
...
class BaseHelper
{
    ...
    public function scanFolder(string $path, array $ignoreFiles = []): array
    {
        try {
            if (File::isDirectory($path)) {
                $data = array_diff(
                    scandir($path),
                    array_merge([".", "..", ".DS_Store"], $ignoreFiles)
                );
                natsort($data);
                return $data;
            }
    ...
    public function getAdminMasterLayoutTemplate(): string
    {
        return apply_filters(
            "base_filter_admin_master_layout_template",
            "core/base::layouts.master"
        );
    }
    ...
    public function adminLanguageDirection(): string
    {
        $direction = session(
            ...
            setting("admin_locale_direction", "ltr")
        );
    ...
    public function getRichEditor(): string
    {
        return setting(
            "rich_editor",
            config("core.base.general.editor.primary")
        );
    }
    ...
    public function sortSearchResults(
        ...
        return $collection->sortByDesc(function ($item) use (
            $searchTerms,
            $column
        ) {
