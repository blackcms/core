...
use BlackCMS\Base\Exceptions\Handler;
use BlackCMS\Base\Facades\MacroableModelsFacade;
use BlackCMS\Base\Http\Middleware\CoreMiddleware;
use BlackCMS\Base\Http\Middleware\DisableInDemoModeMiddleware;
use BlackCMS\Base\Http\Middleware\HttpsProtocolMiddleware;
use BlackCMS\Base\Http\Middleware\LocaleMiddleware;
use BlackCMS\Base\Models\MetaBox as MetaBoxModel;
use BlackCMS\Base\Repositories\Caches\MetaBoxCacheDecorator;
use BlackCMS\Base\Repositories\Eloquent\MetaBoxRepository;
use BlackCMS\Base\Repositories\Interfaces\MetaBoxInterface;
use BlackCMS\Base\Supports\BreadcrumbsManager;
use BlackCMS\Base\Supports\CustomResourceRegistrar;
use BlackCMS\Base\Supports\Helper;
use BlackCMS\Base\Traits\LoadAndPublishDataTrait;
use BlackCMS\Setting\Providers\SettingServiceProvider;
use BlackCMS\Setting\Supports\SettingStore;
use BlackCMS\Support\Http\Middleware\BaseMiddleware;
use DateTimeZone;
...
class BaseServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        $this->app->make("config")->set([
            "session.cookie" => "blackcms_session",
    ...
    {
        $this->loadAndPublishConfigurations(["permissions", "assets"])
            ->loadAndPublishViews()
        ...
        $router->aliasMiddleware(
            "preventDemo",
            DisableInDemoModeMiddleware::class
        );
        ...
        $this->app->booted(function () use ($config) {
            ...
            add_action(
                ...
                [MetaBox::class, "doMetaBoxes"],
                8,
                2
            );

            $setting = $this->app->make(SettingStore::class);
            $timezone = $setting->get(
                "time_zone",
        ...
        $forceUrl = $this->app
            ->make("config")
            ->get("core.base.general.force_root_url");
        if (!empty($forceUrl)) {
            URL::forceRootUrl($forceUrl);
        }

        $forceSchema = $this->app
            ->make("config")
            ->get("core.base.general.force_schema");
    ...
    protected function configureIni()
    {
        ...
        $memoryLimit = $this->app
            ->make("config")
            ->get("core.base.general.memory_limit");
        ...
        if (-1 !== $currentLimitInt &&
            (-1 === $limitInt || $limitInt > $currentLimitInt)
        ) {
