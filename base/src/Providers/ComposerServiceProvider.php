...
class ComposerServiceProvider extends ServiceProvider
{
    ...
    {
        $view->composer(
            ["core/base::layouts.partials.top-header", "core/acl::auth.master"],
            function (View $view) {
                $themes = Assets::getThemes();

                $defaultTheme = setting(
                    "default_admin_theme",
                    config("core.base.general.default-theme")
                );

                if (Auth::check() &&
                    !session()->has("admin-theme") &&
                    !app()->environment("demo")
                ) {
                    $activeTheme = UserMeta::getMeta(
                        "admin-theme",
                        $defaultTheme
                    );
        ...
        $view->composer(["core/media::config"], function () {
            $mediaPermissions = MediaManagement::getConfig("permissions");
            if (Auth::check() && !Auth::user()->isSuperUser()) {
                $mediaPermissions = array_intersect(
                    array_keys(Auth::user()->permissions),
                    $mediaPermissions
                );
            }

            MediaManagement::setPermissions($mediaPermissions);
        });
    }
}
