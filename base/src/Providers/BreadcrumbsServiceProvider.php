...
use BlackCMS\Base\Supports\BreadcrumbsGenerator;
use Breadcrumbs;
...
class BreadcrumbsServiceProvider extends ServiceProvider
{
    ...
    {
        Breadcrumbs::register("dashboard.index", function (
            BreadcrumbsGenerator $breadcrumbs
        ) {
            $breadcrumbs->push(
                trans("core/base::layouts.dashboard"),
         ...
         */
        Breadcrumbs::register("main", function (
            BreadcrumbsGenerator $breadcrumbs,
            $defaultTitle = null
        ) {
            $prefix =
            ...
            $siteTitle = setting(
                "admin_title",
                config("core.base.general.base_name")
            );
            ...
            foreach ($arMenu as $menuCategory) {
                if (($url == $menuCategory["url"] ||
                        (Str::contains($menuCategory["url"], $prefix) &&
                            $prefix != "//")) &&
                    !empty($menuCategory["name"])
                ) {
                    $found = true;
                    $breadcrumbs->push(
                        trans($menuCategory["name"]),
                        $menuCategory["url"]
                    );
            ...
            if (!$found) {
                foreach ($arMenu as $menuCategory) {
                    ...
                    foreach ($menuCategory["children"] as $menuItem) {
                        if (($url == $menuItem["url"] ||
                                (Str::contains($menuItem["url"], $prefix) &&
                                    $prefix != "//")) &&
                            !empty($menuItem["name"])
                        ) {
                            $found = true;
                            ...
                            if ($defaultTitle != trans($menuItem["name"]) &&
                                $defaultTitle != $siteTitle
                            ) {
                                $breadcrumbs->push(
                                    $defaultTitle,
