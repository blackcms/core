...
namespace BlackCMS\Setting\Http\Controllers;

use Assets;
use BlackCMS\Base\Http\Controllers\BaseController;
use BlackCMS\Base\Http\Responses\BaseHttpResponse;
use BlackCMS\Base\Supports\Core;
use BlackCMS\Base\Supports\Language;
use BlackCMS\Media\Repositories\Interfaces\MediaFileInterface;
use BlackCMS\Setting\Http\Requests\EmailTemplateRequest;
use BlackCMS\Setting\Http\Requests\MediaSettingRequest;
use BlackCMS\Setting\Http\Requests\SendTestEmailRequest;
use BlackCMS\Setting\Http\Requests\SettingRequest;
...
class SettingController extends BaseController
{
    ...
    public function postEdit(
        ...
        if ($locale &&
            array_key_exists($locale, Language::getAvailableLocales())
        ) {
        ...
        if (!app()->environment("demo")) {
            setting()
                ->set("default_admin_theme", $adminTheme)
                ->save();
        }
        ...
        if (!app()->environment("demo")) {
            setting()
                ->set("admin_locale_direction", $adminLocalDirection)
                ->save();
        }
     ...
     * @return BaseHttpResponse
     */
    public function postEditEmailConfig(
        SettingRequest $request,
        BaseHttpResponse $response
    ) {
    ...
    public function getEditEmailTemplate($type, $module, $template)
    {
        ...
        $emailContent = get_setting_email_template_content(
            $type,
            $module,
            $template
        );
        ...
        return view(
            "core/setting::email-template-edit",
            compact("emailContent", "emailSubject", "addonData")
        );
    }
    ...
    public function postStoreEmailTemplate(
        ...
        if ($request->has("email_subject_key")) {
            setting()
                ->set(
                    $request->input("email_subject_key"),
     ...
     * @throws Exception
     */
    public function postResetToDefault(
        Request $request,
        BaseHttpResponse $response
    ) {
        $this->settingRepository->deleteBy([
            "key" => $request->input("email_subject_key"),
        ]);
        File::delete($request->input("template_path"));

        return $response->setMessage(
            trans("core/setting::setting.email.reset_success")
        );
    }
     ...
     * @return BaseHttpResponse
     */
    public function postChangeEmailStatus(
        Request $request,
        BaseHttpResponse $response
    ) {
        setting()
    ...
    public function postSendTestEmail(
        ...
        try {
            EmailHandler::send(
                file_get_contents(
                    core_path("setting/resources/email-templates/test.tpl")
                ),
                "Test",
                $request->input("email"),
                [],
            ...
            return $response->setMessage(
                trans("core/setting::setting.test_email_send_success")
            );
    ...
    public function generateThumbnails(
        ...
        if ($errors) {
            return $response->setError()->setMessage(
                trans("core/setting::setting.generate_thumbnails_error", [
                    "count" => count($errors),
                ])
            );
        }
        ...
        return $response->setMessage(
            trans("core/setting::setting.generate_thumbnails_success", [
                "count" => count($files),
            ])
        );
    }
