...
use BlackCMS\Media\Http\Resources\FileResource;
use BlackCMS\Media\Http\Resources\FolderResource;
...
use BlackCMS\Media\Services\UploadsManager;
use BlackCMS\Media\Supports\Zipper;
use Eloquent;
use Exception;
use File;
...
class MediaController extends Controller
{
        ...
        switch ($request->input("view_in")) {
            case "all_media":
                ...
                $folders = FolderResource::collection(
                    $queried->where("is_folder", 1)
                );

                $files = FileResource::collection(
                    $queried->where("is_folder", 0)
                );
            ...
            case "favorites":
                $breadcrumbs = [
                    [
                        ...
                        "icon" => "las la-star la-2x",
                    ],
                ];

                $favoriteItems = $this->mediaSettingRepository->getFirstBy([
                    "key" => "favorites",
                    "user_id" => Auth::id(),
                ]);
                ...
                if (!empty($favoriteItems)) {
                    ...
                    $paramsFile = array_merge_recursive($paramsFile, [
                        "condition" => [["media_files.id", "IN", $fileIds]],
                    ]);

                    $paramsFolder = array_merge_recursive($paramsFolder, [
                        "condition" => [["media_folders.id", "IN", $folderIds]],
                    ]);
    ...
    protected function getBreadcrumbs(Request $request)
    {
        ...
        if ($request->input("view_in") == "trash") {
            $folder = $this->folderRepository->getFirstByWithTrash([
                "id" => $folderId,
            ]);
        } else {
    ...
    {
        $response = MediaManagement::responseError(
            trans("core/media::media.invalid_action")
        );
        ...
        switch ($type) {
            case "trash":
                ...
                if ($error) {
                    $response = MediaManagement::responseError(
                        trans("core/media::media.trash_error")
                    );
                    break;
                }

                $response = MediaManagement::responseSuccess(
                    [],
                    trans("core/media::media.trash_success")
                );
                break;
            ...
            case "restore":
                ...
                if ($error) {
                    $response = MediaManagement::responseError(
                        trans("core/media::media.restore_error")
                    );
                    break;
                }
                ...
                $response = MediaManagement::responseSuccess(
                    [],
                    trans("core/media::media.restore_success")
                );
                break;
            ...
            case "make_copy":
                foreach ($request->input("selected", []) as $item) {
                    $id = $item["id"];
                    if ($item["is_folder"] == "false") {
                        $file = $this->fileRepository->getFirstBy([
                            "id" => $id,
                        ]);
                        ...
                        $oldFolder = $this->folderRepository->getFirstBy([
                            "id" => $id,
                        ]);
                        ...
                        $folderData[
                            "slug"
                        ] = $this->folderRepository->createSlug(
                            $oldFolder->name,
                            $oldFolder->parent_id
                        );
                        ...
                        $files = $this->fileRepository->getFilesByFolderId(
                            $id,
                            [],
                            false
                        );
                        ...
                        $children = $this->folderRepository->getAllChildFolders(
                            $id
                        );
                        foreach ($children as $parentId => $child) {
                            if ($parentId != $oldFolder->id) {
                                ...
                                $parentFiles = $this->fileRepository->getFilesByFolderId(
                                    $parentId,
                                    [],
                                    false
                                );
                            ...
                            foreach ($child as $sub) {
                                 ...
                                 * @var Eloquent $sub
                                 */
                                $subFiles = $this->fileRepository->getFilesByFolderId(
                                    $sub->id,
                                    [],
                                    false
                                );
                                ...
                                $sub = $this->folderRepository->create(
                                    $subFolderData
                                );
                        ...
                        $allFiles = Storage::allFiles(
                            $this->folderRepository->getFullPath($oldFolder->id)
                        );
                ...
                $response = MediaManagement::responseSuccess(
                    [],
                    trans("core/media::media.copy_success")
                );
                break;
            ...
            case "delete":
                ...
                $response = MediaManagement::responseSuccess(
                    [],
                    trans("core/media::media.delete_success")
                );
                break;
            ...
            case "favorite":
                ...
                $response = MediaManagement::responseSuccess(
                    [],
                    trans("core/media::media.favorite_success")
                );
                break;
            ...
            case "remove_favorite":
                ...
                $response = MediaManagement::responseSuccess(
                    [],
                    trans("core/media::media.remove_favorite_success")
                );
                break;
            ...
            case "rename":
                ...
                foreach ($request->input("selected") as $item) {
                    ...
                    if ($item["is_folder"] == "false") {
                        ...
                        if (!empty($folder)) {
                            $folder->name = $this->folderRepository->createName(
                                $name,
                                $folder->parent_id
                            );
                ...
                if (!empty($error)) {
                    $response = MediaManagement::responseError(
                        trans("core/media::media.rename_error")
                    );
                    break;
                }
                ...
                $response = MediaManagement::responseSuccess(
                    [],
                    trans("core/media::media.rename_success")
                );
                break;
            ...
            case "empty_trash":
                ...
                $response = MediaManagement::responseSuccess(
                    [],
                    trans("core/media::media.empty_trash_success")
                );
                break;
        }
    ...
    protected function copyFile($file, $newFolderId = null)
    {
        ...
        if ($newFolderId == null) {
            ...
            $filePath = MediaManagement::getRealPath($file->url);
            if (Storage::exists($filePath)) {
                ...
                MediaManagement::generateThumbnails($file);
            }
        } else {
            $file->url = str_replace(
                MediaManagement::getRealPath(File::dirname($file->url)),
                MediaManagement::getRealPath(
                    $this->folderRepository->getFullPath($newFolderId)
                ),
                $file->url
            );
    ...
    public function download(Request $request)
    {
        ...
        if (count($items) == 1 && $items["0"]["is_folder"] == "false") {
            $file = $this->fileRepository->getFirstByWithTrash([
                "id" => $items[0]["id"],
            ]);
            if (!empty($file) && $file->type != "video") {
                $filePath = MediaManagement::getRealPath($file->url);
                if (!MediaManagement::isUsingCloud()) {
                    if (!File::exists($filePath)) {
                        return MediaManagement::responseError(
                            trans("core/media::media.file_not_exists")
                        );
                    }
                ...
                return response()->make(
                    file_get_contents(
                        str_replace("https://", "http://", $filePath)
                    ),
                    200,
                    [
            ...
            $fileName = MediaManagement::getRealPath(
                "download-" . now()->format("Y-m-d-h-i-s") . ".zip"
            );
            $zip = new Zipper();
            $zip->make($fileName);
                ...
                if ($item["is_folder"] == "false") {
                    ...
                    if (!empty($folder)) {
                        if (!MediaManagement::isUsingCloud()) {
                            $zip->add(
                                MediaManagement::getRealPath(
                                    $this->folderRepository->getFullPath(
                                        $folder->id
                                    )
                                )
                            );
                                ...
                                $zip->addString(
                                    ...
                                    file_get_contents(
                                        str_replace(
                                            "https://",
                                            "http://",
                                            MediaManagement::getRealPath($file)
                                        )
                                    )
                                );
                            }
            ...
            return MediaManagement::responseError(
                trans("core/media::media.download_file_error")
            );
        }

        return MediaManagement::responseError(
            trans("core/media::media.can_not_download_file")
        );
    }
}
