...
namespace BlackCMS\Media\Services;
...
class UploadsManager
{
    ...
    public function fileMimeType(string $path): ?string
    {
        return $this->mimeType->getMimeType(
            File::extension(MediaManagement::getRealPath($path))
        );
    }
    ...
    protected function cleanFolder(string $folder): string
    {
        return DIRECTORY_SEPARATOR .
            trim(str_replace("..", "", $folder), DIRECTORY_SEPARATOR);
    }
    ...
    public function saveFile(
        ...
        if (!MediaManagement::isChunkUploadEnabled() || !$file) {
            return Storage::put($this->cleanFolder($path), $content);
        }

        $currentChunksPath =
            MediaManagement::getConfig("chunk.storage.chunks") .
            "/" .
            $file->getFilename();
        $disk = Storage::disk(MediaManagement::getConfig("chunk.storage.disk"));
