...
class MediaManagement
{
    ...
    public function getUrls(): array
    {
        return [
            ...
            "media_upload_from_editor" => route(
    ...
    public function getSizes(): array
    {
        ...
        foreach ($sizes as $name => $size) {
            ...
            $sizes[$name] =
                setting($settingName . "_width", $size[0]) .
                "x" .
                setting($settingName . "_height", $size[1]);
        }
    ...
    public function url(?string $path): string
    {
        ...
        if (Str::contains($path, "https://") ||
            Str::contains($path, "http://")
        ) {
            return $path;
        }

        if (config("filesystems.default") === "do_spaces" &&
            (int) setting("media_do_spaces_cdn_enabled")
        ) {
    ...
    public function uploadFromEditor(
        ...
        if ($validator->fails()) {
            return response(
                '<script>alert("' .
                    trans("core/media::media.can_not_detect_file_type") .
                    '")</script>'
        ...
        return response(
            '<script>alert("' . Arr::get($result, "message") . '")</script>'
        )->header("Content-Type", "text/html");
    }
    ...
    public function handleUpload(
        ...
        if ($request->input("path")) {
            $folderId = $this->handleTargetFolder(
                $folderId,
                $request->input("path")
            );
        }
        ...
        if (!$fileUpload) {
            return [
                "error" => true,
                "message" => trans(
                    "core/media::media.can_not_detect_file_type"
                ),
            ];
        }
        ...
        if (!$this->isChunkUploadEnabled()) {
            if (!$skipValidation) {
                $validator = Validator::make(
                    ...
                    [
                        "uploaded_file" =>
                            "required|mimes:" . $allowedMimeTypes,
                    ]
                );
        ...
        try {
            ...
            if ($folderId == 0 && !empty($folderSlug)) {
                ...
                if (!$folder) {
                    $folder = $this->folderRepository->createOrUpdate([
                        ...
                        "name" => $this->folderRepository->createName(
                            $folderSlug,
                            0
                        ),
                        "slug" => $this->folderRepository->createSlug(
                            $folderSlug,
                            0
                        ),
                        "parent_id" => 0,
            ...
            $folderPath =
                $folderId == 0 && empty($folderSlug)
                    ? "/"
                    : $this->folderRepository->getFullPath($folderId);
    ...
    public function generateThumbnails(MediaFile $file): bool
    {
        ...
        $folderIds = json_decode(
            setting("media_folders_can_add_watermark"),
            true
        );
    ...
    public function insertWatermark(string $image): bool
    {
        if (!$image ||
            !setting(
                ...
                $this->getConfig("watermark.enabled")
            )
        ) {
            return false;
        }

        $watermarkImage = setting(
        ...
        $watermark
            ...
            ->opacity(
                (int) setting(
                    ...
                    $this->getConfig("watermark.opacity")
                )
            );
        ...
        $imageSource->insert(
            ...
            setting(
                ...
                $this->getConfig("watermark.position")
            ),
            (int) setting(
                "watermark_position_x",
                $this->getConfig("watermark.x")
            ),
            (int) setting(
                "watermark_position_y",
                $this->getConfig("watermark.y")
            )
        );
        ...
        $this->uploadManager->saveFile(
            $destinationPath,
            $imageSource->stream()->__toString()
        );
     ...
     * @return string
     */
    public function handleTargetFolder(
        ?int $folderId = 0,
        string $filePath = ""
    ): string {
