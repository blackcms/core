...
namespace BlackCMS\Media\Repositories\Eloquent;
 ...
 */
class MediaFileRepository extends RepositoriesAbstract implements
    MediaFileInterface
{
    /**
     * {@inheritDoc}
    ...
    public function createSlug($name, $extension, $folderPath)
    {
        $slug = Str::slug(
            $name,
            "-",
            !MediaManagement::turnOffAutomaticUrlTranslationIntoLatin()
                ? "en"
                : false
        );
        $index = 1;
        ...
        while (File::exists(
            MediaManagement::getRealPath(
                rtrim($folderPath, "/") . "/" . $slug . "." . $extension
            )
        )
        ) {
            $slug = $baseSlug . "-" . $index++;
    ...
    public function getFilesByFolderId(
        ...
        if ($withFolders) {
            ...
            $this->model = $this->model->union($folder);
        }
        ...
        if (isset($params["recent_items"]) &&
            is_array($params["recent_items"]) &&
            $params["recent_items"]
        ) {
            $this->model = $this->model->whereIn(
                "media_files.id",
                Arr::get($params, "recent_items", [])
            );
        }
        ...
        if ($params["selected_file_id"]) {
            if (!$params["paginate"]["current_paged"] ||
