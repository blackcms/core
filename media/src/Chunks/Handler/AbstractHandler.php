...
namespace BlackCMS\Media\Chunks\Handler;
...
abstract class AbstractHandler
{
    ...
    public function __construct(Request $request, $file)
    {
        ...
        $this->file = $file;
        $this->config = MediaManagement::getConfig("chunk");
    }
    ...
    public function createChunkFileName(
        ...
        $array = [$this->file->getClientOriginalName()];
        ...
        if ($useBrowser) {
            $array[] = md5(
                $this->request->ip() .
                    $this->request->header("User-Agent", "no-browser")
            );
        }
