...
namespace BlackCMS\Media\Chunks\Receiver;

use BlackCMS\Media\Chunks\Exceptions\UploadFailedException;
use BlackCMS\Media\Chunks\Handler\AbstractHandler;
use BlackCMS\Media\Chunks\Save\AbstractSave;
...
class FileReceiver
{
    ...
    public function __construct(
        ...
        $this->file = is_object($fileIndexOrFile)
            ? $fileIndexOrFile
            : $request->file($fileIndexOrFile);
        $this->chunkStorage =
            $chunkStorage === null ? ChunkStorage::storage() : $chunkStorage;
    ...
    public function isUploaded()
    {
        return is_object($this->file) &&
            UPLOAD_ERR_NO_FILE !== $this->file->getError();
    }
