...
namespace BlackCMS\Media\Supports;
...
class Zipper
{
    ...
    protected function createArchiveFile($pathToZip)
    {
        if (!$this->file->exists($pathToZip)) {
            ...
            if (!$this->file->exists($dirname) &&
                ...
                throw new Exception(
                    sprintf('The path "%s" is not writeable', $pathToZip)
                );
            }
    ...
    protected function addDir($pathToDir)
    {
        ...
        foreach ($this->file->directories($pathToDir) as $dir) {
            ...
            $this->currentFolder = empty($this->currentFolder)
                ? basename($dir)
                : $this->currentFolder . "/" . basename($dir);
