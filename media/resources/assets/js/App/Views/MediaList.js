...
export class MediaList {
    constructor() {
        this.group = {};
        this.group.list = $("#media_management_items_list").html();
        this.group.tiles = $("#media_management_items_tiles").html();

        this.item = {};
        this.item.list = $("#media_management_items_list_element").html();
        this.item.tiles = $("#media_management_items_tiles_element").html();

        this.$groupContainer = $(".media-management-items");
    }
    ...
    renderData(data, reload = false, load_more_file = false) {
        ...
        template = template
            .replace(
                /__noItemIcon__/gi,
                MEDIA_MANAGEMENT_CONFIG.translations.no_item[view_in].icon || ""
            )
            .replace(
                /__noItemTitle__/gi,
                MEDIA_MANAGEMENT_CONFIG.translations.no_item[view_in].title ||
                    ""
            )
            .replace(
                /__noItemMessage__/gi,
                MEDIA_MANAGEMENT_CONFIG.translations.no_item[view_in].message ||
                    ""
            );
        ...
        if (
            load_more_file &&
            this.$groupContainer.find(".media-management-grid ul").length > 0
        ) {
            $itemsWrapper = this.$groupContainer.find(
                ".media-management-grid ul"
            );
        }

        if (
            _.size(data.folders) > 0 ||
            _.size(data.files) > 0 ||
            load_more_file
        ) {
            $(".media-management-items").addClass("has-items");
        } else {
            $(".media-management-items").removeClass("has-items");
        }
        ...
        _.forEach(data.files, (value) => {
            ...
            if (Helpers.getRequestParams().view_type === "list") {
                ...
                item = item.replace(
                    ...
                    value.type === "image"
                        ? '<img src="' +
                              (value.thumb ? value.thumb : value.full_url) +
                              '" alt="' +
                              value.name +
                              '">'
                        : '<i class="' + value.icon + '"></i>'
        ...
        if (
            !(
                load_more_file &&
                this.$groupContainer.find(".media-management-grid ul").length >
                    0
            )
        ) {
        ...
        $(
            ".js-media-list-title[data-id=" + data.selected_file_id + "]"
        ).trigger("click");
    }
}
