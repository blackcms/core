...
export class MediaDetails {
    constructor() {
        this.$detailsWrapper = $(
            ".media-management-main .media-management-details"
        );

        this.descriptionItemTemplate =
            '<div class="media-management-name"><p>__title__</p>__url__</div>';
    ...
    renderData(data) {
            ...
            if (_.includes(_self.onlyFields, index)) {
                if (!_.includes(["size", "mime_type"], index)) {
                    description += _self.descriptionItemTemplate
                        .replace(
                            /__title__/gi,
                            MEDIA_MANAGEMENT_CONFIG.translations[index]
                        )
                        .replace(
                            /__url__/gi,
                            val
        ...
        _self.$detailsWrapper.find(".media-management-thumbnail").html(thumb);
        _self.$detailsWrapper
            .find(".media-management-description")
            .html(description);
        if (useClipboard) {
