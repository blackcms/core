...
export class UploadService {
    constructor() {
        ...
        this.uploadUrl = MEDIA_MANAGEMENT_URL.upload_file;
        ...
        this.uploadProgressTemplate = $(
            "#media_management_upload_progress_item"
        ).html();
    ...
    init() {
        if (
            _.includes(MEDIA_MANAGEMENT_CONFIG.permissions, "files.create") &&
            $(".media-management-items").length > 0
        ) {
            this.setupDropZone();
        }
        this.handleEvents();
    ...
    setupDropZone() {
        ...
        _self.dropZone = new Dropzone(
            document.querySelector(".media-management-items"),
            {
                ..._dropZoneConfig,
                ...
                sending: function (file, xhr, formData) {
                    ...
                    formData.append(
                        "folder_id",
                        Helpers.getRequestParams().folder_id
                    );
                    formData.append(
                        "view_in",
                ...
                uploadprogress: (file, progress, bytesSent) => {
                    ...
                    let percentShow =
                        (percent > 100 ? "100" : parseInt(percent)) + "%";
                    let el = _self.uploadProgressContainer
                        .find("li")
                        .eq(file.index - 1);
    ...
    changeProgressStatus(file) {
        let _self = this;
        let $progressLine = _self.uploadProgressContainer.find(
            "li:nth-child(" + file.index + ")"
        );
        ...
        _self.totalError =
            _self.totalError +
            (response.error === true || file.status === "error" ? 1 : 0);

        $label.addClass(
            response.error === true || file.status === "error"
                ? "label-danger"
                : "label-success"
        );
        $label.html(
    ...
    getDropZoneConfig() {
        return {
            url: this.uploadUrl,
            uploadMultiple: !MEDIA_MANAGEMENT_CONFIG.chunk.enabled,
            chunking: MEDIA_MANAGEMENT_CONFIG.chunk.enabled,
            ...
            chunkSize: MEDIA_MANAGEMENT_CONFIG.chunk.chunk_size, // chunk size 1,000,000 bytes (~1MB)
            retryChunks: true, // retry chunks on failure
            retryChunksLimit: 3, // retry maximum of 3 times (default is 3)
            timeout: 0, // MB,
            maxFilesize: MEDIA_MANAGEMENT_CONFIG.chunk.max_file_size, // MB
