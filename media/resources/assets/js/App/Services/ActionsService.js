...
export class ActionsService {
    ...
    static handleCopyLink() {
        ...
        MessageService.showMessage(
            "success",
            MEDIA_MANAGEMENT_CONFIG.translations.clipboard.success,
            MEDIA_MANAGEMENT_CONFIG.translations.message.success_header
        );
    ...
    static handleGlobalAction(type, callback) {
        ...
        switch (type) {
            ...
            case "empty_trash":
                ...
                break;
            case "download":
                let downloadLink = MEDIA_MANAGEMENT_URL.download;
                let count = 0;
                _.each(Helpers.getSelectedItems(), (value) => {
                    if (
                        !_.includes(
                            Helpers.getConfigs().denied_download,
                            value.mime_type
                        )
                    ) {
                        downloadLink +=
                ...
                if (downloadLink !== MEDIA_MANAGEMENT_URL.download) {
                    window.open(downloadLink, "_blank");
                } else {
                    MessageService.showMessage(
                        "error",
                        MEDIA_MANAGEMENT_CONFIG.translations.download.error,
                        MEDIA_MANAGEMENT_CONFIG.translations.message
                            .error_header
                    );
                }
                break;
    ...
    static processAction(data, callback = null) {
        $.ajax({
            url: MEDIA_MANAGEMENT_URL.global_actions,
            type: "POST",
            data: data,
            dataType: "json",
            beforeSend: () => {
    ...
    static renderRenameItems() {
        let VIEW = $("#media_management_rename_item").html();
    ...
    static renderActions() {
        ...
        if (hasFolderSelected) {
            ...
            if (
                !_.includes(
                    MEDIA_MANAGEMENT_CONFIG.permissions,
                    "folders.create"
                )
            ) {
                actionsList.file = _.reject(actionsList.file, (item) => {
            ...
            if (
                !_.includes(MEDIA_MANAGEMENT_CONFIG.permissions, "folders.edit")
            ) {
                actionsList.file = _.reject(actionsList.file, (item) => {
        ...
        if (selectedFiles.length > 0) {
            ...
            if (
                !_.includes(MEDIA_MANAGEMENT_CONFIG.permissions, "files.edit")
            ) {
                actionsList.file = _.reject(actionsList.file, (item) => {
                    return _.includes(["rename"], item.action);
                });
            ...
            if (
                !_.includes(MEDIA_MANAGEMENT_CONFIG.permissions, "files.trash")
            ) {
                actionsList.other = _.reject(actionsList.other, (item) => {
                    return _.includes(["trash", "restore"], item.action);
                });
        ...
        _.each(actionsList, (action, key) => {
            _.each(action, (item, index) => {
                ...
                if (!is_break) {
                    let template = ACTION_TEMPLATE.replace(
                        /__action__/gi,
                        item.action || ""
                    )
                        ...
                        .replace(
                            /__name__/gi,
                            MEDIA_MANAGEMENT_CONFIG.translations.actions_list[
                                key
