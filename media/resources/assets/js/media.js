...
class MediaManagement {
    ...
    setupLayout() {
         ...
         */
        let $current_filter = $(
            '.js-media-management-change-filter[data-type="filter"][data-value="' +
                Helpers.getRequestParams().filter +
                '"]'
        );
        ...
        $current_filter
            ...
            .addClass("active")
            .closest(".dropdown")
            .find(".js-media-management-filter-current")
            .html("(" + $current_filter.html() + ")");

        let $current_view_in = $(
            '.js-media-management-change-filter[data-type="view_in"][data-value="' +
                Helpers.getRequestParams().view_in +
                '"]'
        );
        ...
        $current_view_in
            ...
            .closest(".dropdown")
            .find(".js-media-management-filter-current")
            .html("(" + $current_view_in.html() + ")");

        if (Helpers.isUseInModal()) {
            $(".media-management-footer").removeClass("hidden");
        }
         ...
         */
        $(
            '.js-media-management-change-filter[data-type="sort_by"][data-value="' +
                Helpers.getRequestParams().sort_by +
                '"]'
        )
            .closest("li")
            .addClass("active");
        ...
        $mediaDetailsCheckbox.prop(
            "checked",
            MediaConfig.hide_details_pane || false
        );

        setTimeout(() => {
            $(".media-management-details").removeClass("hidden");
        }, 300);

        $mediaDetailsCheckbox.on("change", (event) => {
            event.preventDefault();
            MediaConfig.hide_details_pane = $(event.currentTarget).is(
                ":checked"
            );
            Helpers.storeConfig();
        });
    ...
    handleMediaList() {
        ...
        _self.$body
            ...
            .on("click", ".js-media-list-title", (event) => {
                ...
                if (shift_key) {
                    ...
                    if (firstItem) {
                        ...
                        $(".media-management-items li").each((index, el) => {
                    ...
                    $current
                        .closest(".media-management-items")
                        .find("input[type=checkbox]")
                        .prop("checked", false);
                }
            ...
            .on("dblclick", ".js-media-list-title", (event) => {
                ...
                if (data.is_folder === true) {
                    ...
                    if (!Helpers.isUseInModal()) {
                        ...
                        Helpers.getConfigs().request_params.view_in !== "trash"
                    ) {
            ...
            .on("dblclick", ".js-up-one-level", (event) => {
                event.preventDefault();
                let count = $(
                    ".media-management-breadcrumb .breadcrumb li"
                ).length;
                $(
                    ".media-management-breadcrumb .breadcrumb li:nth-child(" +
                        (count - 1) +
                        ") a"
                ).trigger("click");
            })
            ...
            .on("click contextmenu", ".media-management-items", (e) => {
                if (!_.size(e.target.closest(".js-context-menu"))) {
                    $('.media-management-items input[type="checkbox"]').prop(
                        "checked",
                        false
                    );
    ...
    changeViewType() {
        let _self = this;
        _self.$body
            .off("click", ".js-media-management-change-view-type .btn")
            .on(
                "click",
                ".js-media-management-change-view-type .btn",
                (event) => {
                    event.preventDefault();
                    let $current = $(event.currentTarget);
                    ...
                    $current
                        .closest(".js-media-management-change-view-type")
                        .find(".btn")
                        .removeClass("active");
                    $current.addClass("active");

                    MediaConfig.request_params.view_type =
                        $current.data("type");
                    ...
                    if (
                        typeof MEDIA_MANAGEMENT_CONFIG.pagination != "undefined"
                    ) {
                        if (
        ...
        $(
            '.js-media-management-change-view-type .btn[data-type="' +
                Helpers.getRequestParams().view_type +
                '"]'
        ).trigger("click");
    ...
    changeFilter() {
        let _self = this;
        _self.$body
            .off("click", ".js-media-management-change-filter")
            .on("click", ".js-media-management-change-filter", (event) => {
                event.preventDefault();
    ...
    search() {
        ...
        $('.input-search-wrapper input[type="text"]').val(
            Helpers.getRequestParams().search || ""
        );
        _self.$body
            .off("submit", ".input-search-wrapper")
            .on("submit", ".input-search-wrapper", (event) => {
                event.preventDefault();
                MediaConfig.request_params.search = $(event.currentTarget)
    ...
    handleActions() {
        ...
        _self.$body
            .off(
                "click",
                '.media-management-actions .js-change-action[data-type="refresh"]'
            )
            .on(
                "click",
                '.media-management-actions .js-change-action[data-type="refresh"]',
                (event) => {
                    ...
                    let ele_options =
                        typeof window.mediaManagement.$el !== "undefined"
                            ? window.mediaManagement.$el.data(
                                  "media-management"
                              )
                            : undefined;
                    if (
            ...
            .off("click", ".media-management-items li.no-items")
            .on("click", ".media-management-items li.no-items", (event) => {
                event.preventDefault();
                $(
                    ".media-management-header .media-management-top-header .media-management-actions .js-dropzone-upload"
                ).trigger("click");
            })
            ...
            .on("click", ".js-files-action", (event) => {
                event.preventDefault();
                ActionsService.handleGlobalAction(
                    $(event.currentTarget).data("action"),
                    () => {
                        Helpers.resetPagination();
                        _self.MediaService.getMedia(true);
                    }
            ...
            .on("submit", ".form-download-url", async (event) => {
                ...
                await _self.DownloadService.download(
                    ...
                    `);
                        $notice
                            .append($noticeItem)
                            .scrollTop($notice[0].scrollHeight);
                        $header.html(
                            `<i class="fas fa-cloud-download-alt"></i> ${$header.data(
                                "downloading"
                            )} (${progress})`
                        );
                        return (success, message = "") => {
                            ...
                            $noticeItem
                                .attr(
                                    "class",
                                    `p-2 text-${success ? "success" : "danger"}`
                                )
                                .find("i")
                                .attr(
                                    "class",
    ...
    handleModals() {
        ...
        _self.$body
            ...
            .on("submit", ".form-delete-items", (event) => {
                event.preventDefault();
                let items = [];
                let $form = $(event.currentTarget);
        ...
        _self.$body
            ...
            .on("submit", "#modal_empty_trash .rv-form", (event) => {
                event.preventDefault();
                let $form = $(event.currentTarget);
    ...
    checkFileTypeSelect(selectedFiles) {
        if (typeof window.mediaManagement.$el !== "undefined") {
            ...
            if (
                ...
                if (!ele_options[0].file_type.match(firstItem.type)) {
                    ...
                    if (
                        typeof ele_options[0].ext_allowed !== "undefined" &&
                        $.isArray(ele_options[0].ext_allowed)
                    ) {
                        if (
                            $.inArray(
                                firstItem.mime_type,
                                ele_options[0].ext_allowed
                            ) === -1
                        ) {
                            return false;
                        }
    ...
    bindIntegrateModalEvents() {
        let $mainModal = $("#media_management_modal");
        let _self = this;
        $mainModal
            .off("click", ".js-insert-to-editor")
            .on("click", ".js-insert-to-editor", (event) => {
                event.preventDefault();
                let selectedFiles = Helpers.getSelectedFiles();
                if (_.size(selectedFiles) > 0) {
                    window.mediaManagement.options.onSelectFiles(
                        selectedFiles,
                        window.mediaManagement.$el
                    );
    ...
    scrollGetMore() {
        let _self = this;
        $(".media-management-main .media-management-items").bind(
                    ...
                    if (loadMore) {
                        if (
                            typeof MEDIA_MANAGEMENT_CONFIG.pagination !=
                                "undefined" &&
                            MEDIA_MANAGEMENT_CONFIG.pagination.has_more
                        ) {
