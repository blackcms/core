...
namespace BlackCMS\Support\Repositories\Eloquent;

use BlackCMS\Base\Supports\RepositoryHelper;
...
abstract class RepositoriesAbstract implements RepositoryInterface
{
    ...
    {
        $data = RepositoryHelper::applyBeforeExecuteQuery(
            $data,
            $this->originalModel,
            $isSingle
        );
    ...
    public function advancedGet(array $params = [])
    {
        ...
        if ($params["take"] == 1) {
            ...
            if (Arr::get($params, "paginate.type") &&
                method_exists($data, Arr::get($params, "paginate.type"))
            ) {
                $paginateType = Arr::get($params, "paginate.type");
            }

            $result = $this->applyBeforeExecuteQuery($data)->$paginateType(
                (int) Arr::get($params, "paginate.per_page", 10),
                [
                    $this->originalModel->getTable() .
                    "." .
                    $this->originalModel->getKeyName(),
                ],
                "page",
